...
def adaptive_clip_inputs_inplace(model_update, clipping_norm):
    """
    ...
    """
    input_norm = np.linalg.norm([np.linalg.norm(layer) for layer in model_update])

    # Log the input norm and clipping norm
    logging.debug(f"Input norm: {input_norm}, Clipping norm: {clipping_norm}")

    if clipping_norm <= 0:
        logging.error("Clipping norm must be positive. Check initial_clipping_norm value.")
        raise ValueError("Clipping norm must be positive.")

    if input_norm == 0:
        logging.warning("Input norm is zero. Skipping clipping.")
        return 0  # Clipping not applied
